...
const extractWireMovement = (input) => {
  const wire = [];

  input.forEach((element) => {
    const moveDetails = parseMovement(element);
    wire.push(moveDetails[1]);
  });

  return wire;
};

const extractWireCoordinates = (input) => {
  const wire = [];
  let x = 0;
  let y = 0;
    ...
    for (let wire2Index = 1; wire2Index < wire2.length; wire2Index++) {
      ...
      if (status) {
        intersectionPoints.push({
          intersectionPoint :pointOfIntersect,
          wire1Detail: {
            intersectionPoint: wire1Points[0],
            index: wire1Index - 1 
          },
          wire2Detail: {
            intersectionPoint: wire2Points[0],
            index: wire2Index - 1 
          }
        });
      }
...
const extractStepMoveFromOnePointToAnother = (point1, point2) => {
  return point1[0] === point2[0] 
    ? Math.abs(point1[1] - point2[1])
    : Math.abs(point1[0] - point2[0])
}

const calculateSumOfSteps = (wireSteps, pointOfIntersection, wire) => {
  let sum = 0;

  for (let i = 0; i < wire.index; i++) {
    sum += wireSteps[i];
  }

  const wire1Point = wire.intersectionPoint;
  sum += extractStepMoveFromOnePointToAnother(pointOfIntersection, wire1Point);
  console.log(sum);
  
  return sum;
}

const calculateClosestStepsTaken = (wire1Steps, wire2Steps, intersectionData) => {
  const sumOfSteps = [];

  intersectionData.forEach((element) => {
    const intersectionPoint = element.intersectionPoint;
    const sumOfWire1Step = calculateSumOfSteps(wire1Steps, intersectionPoint, element.wire1Detail);
    
    const sumOfWire2Step = calculateSumOfSteps(wire2Steps, intersectionPoint, element.wire2Detail);
    const sum = sumOfWire1Step + sumOfWire2Step;
    sumOfSteps.push(sum);
  })
  console.log(sumOfSteps);
  
  const sortedSteps = sumOfSteps.sort((a,b) => a - b);
  console.log(sortedSteps[0]);
...
const calculateClosestIntersection = (input1, input2) => {
  ...
  const wire1Coordinate = extractWireCoordinates(wire1);
  const wire2Coordinate = extractWireCoordinates(wire2);
  ...
  const wire1Movement = extractWireMovement(wire1);
  const wire2Movement = extractWireMovement(wire2);
  
  intersectionPoints.shift();
  wire1Coordinate.shift();
  wire2Coordinate.shift();

  console.log(wire1Movement);
  console.log(wire1Movement);

  console.log(wire1Movement);
  console.log(wire1Movement);
  
  calculateClosestStepsTaken(wire1Movement, wire2Movement, intersectionPoints);
