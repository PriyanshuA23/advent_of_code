const dbg = (description = "", x) => console.log(description, x);
...
const jumpIfTrue = (intCode, parameters, modes, index) => {
  const parameter1 = extractValueOfParameter(parameters[0], modes[0], intCode);
  const parameter2 = extractValueOfParameter(parameters[1], modes[1], intCode);

  if (parameter1 !== 0) {
    return Math.abs(parameter2 - index);
  }

  return 3;
};

const jumpIfFalse = (intCode, parameters, modes, index) => {
  const parameter1 = extractValueOfParameter(parameters[0], modes[0], intCode);
  const parameter2 = extractValueOfParameter(parameters[1], modes[1], intCode);
...
const lessThan = (intCode, parameters, modes) => {
  const parameter1 = extractValueOfParameter(parameters[0], modes[0], intCode);
  const parameter2 = extractValueOfParameter(parameters[1], modes[1], intCode);
...
const equals = (intCode, parameters, modes) => {
  const parameter1 = extractValueOfParameter(parameters[0], modes[0], intCode);
  const parameter2 = extractValueOfParameter(parameters[1], modes[1], intCode);

  // dbg("parameter1", parameter1);
  // dbg("parameter2", parameter2);
  // dbg("parameter3", parameters[2]);
...
const evaluateIntCode = (input) => { //10099
  ...
  for (let i = 0; i < intCode.length;) {
    ...
    i += operation(intCode, operands, mode, i);
  }
};
